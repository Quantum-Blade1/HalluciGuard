...
import httpx
from src.core.config import CVE_CACHE_DIR
...
class CVEClient:
    ...
    async def has_vulnerabilities(self, package_name: str, ecosystem: str = "PyPI"):
        count, ids = await self.check_cve(package_name, ecosystem)
        from dataclasses import dataclass
        @dataclass
        class CVEResult:
            has_vulnerabilities: bool
            vulnerability_count: int
        return CVEResult(has_vulnerabilities=(count > 0), vulnerability_count=count)
