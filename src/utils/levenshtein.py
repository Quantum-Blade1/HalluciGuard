...
from src.core.config import POPULAR_PYPI_PACKAGES, POPULAR_NPM_PACKAGES
...
# -----------------------------------------------------------------------------
# Backwards compatibility wrappers for ProfilerAgent
# -----------------------------------------------------------------------------
def find_typosquat_candidates(
    ...
    candidates = [
        (match, score) for match, score, _idx in results if match.lower() != name_lower
    ]
